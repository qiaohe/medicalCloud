...
module.exports = {
    ...
    findWaitOutpatients: function (doctorId, registerDate, page, conditions) {
        ...
        sql = !conditions.length ? sql : 'select SQL_CALC_FOUND_ROWS concat(DATE_FORMAT(r.registerDate, \'%Y-%m-%d \') , sp.`name`) as registerDate, d.clinic,r.id, r.patientName, r.patientMobile, r.gender, r.age, r.sequence, r.registrationType, r.`comment`,r.patientId, r.outPatientType, r.createDate,d.id as doctorId, r.businessPeopleName as recommender, r.outpatientStatus, pi.balance, pi.memberType, pi.memberCardNo, r.patientId from Registration r LEFT JOIN  PatientBasicInfo p on p.id = r.patientBasicInfoId left JOIN ShiftPeriod sp on sp.id = r.shiftPeriod LEFT JOIN Patient pi on pi.id = r.patientId LEFT JOIN Doctor d ON d.id = r.doctorId where d.employeeId = ? and r.sequence is not null and r.registerDate=? and ' + conditions.join(' and ') + ' order by field(r.outpatientStatus, 5, 0, 1), r.shiftPeriod, r.createDate limit ?,?';
