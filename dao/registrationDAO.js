...
module.exports = {
    ...
    findRegistrationsBy: function (hospitalId, registrationDate, conditions, page) {
        ...
        sql = !conditions.length ? sql : 'select SQL_CALC_FOUND_ROWS ot.name as outPatientServiceTypeName,r.content,p.medicalRecordNo, r.outPatientType, r.id, r.transferDoctor,r.transferDoctorName,r.outpatientStatus,r.outPatientServiceType,r.patientMobile,r.patientName,r.gender, p.balance, p.memberCardNo, r.memberType, r.doctorName, r.`comment`, r.registrationFee, r.registrationType, r.departmentName, r.registerDate , s.`name` as shiftPeriod, r.outPatientType, r.status, r.sequence, e.name as businessPeopleName, r.status as preRegistrationStatus from Registration r LEFT JOIN Employee e on e.id=r.businessPeopleId left JOIN ShiftPeriod s ON s.id= r.shiftPeriod left join Patient p on r.patientId =p.id  left join OutpatientServiceType ot on ot.id=r.outPatientServiceType where r.hospitalId = ? and r.registerDate=? and ' + conditions.join(' and ') + ' order by r.id desc limit ?, ?';
