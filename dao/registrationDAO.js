...
module.exports = {
    findRegistrations: function (hospitalId, conditions, page) {
        ...
        sql = !conditions.length ? sql : 'select SQL_CALC_FOUND_ROWS mo.orderNo, r.id, r.createDate, r.outpatientStatus, r.patientMobile,r.patientName,r.gender, p.balance, p.source, p.memberCardNo, r.memberType, r.doctorName, r.`comment`, r.registrationFee, r.registrationType, r.departmentName, concat(DATE_FORMAT(r.registerDate, \'%Y-%m-%d \') , s.`name`) as registerDate, r.outPatientType,r.status, r.sequence, e.name as businessPeopleName, r.status as preRegistrationStatus from Registration r LEFT JOIN Employee e on e.id=r.businessPeopleId left JOIN ShiftPeriod s ON s.id= r.shiftPeriod left join Patient p on p.id=r.patientId left join Doctor d on d.id=r.doctorId left join MedicalOrder mo on mo.registrationId= r.id where r.patientId =p.id and r.hospitalId = ? and ' + conditions.join(' and ') + ' order by r.id desc limit ?, ?';
