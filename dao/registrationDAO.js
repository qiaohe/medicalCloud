...
module.exports = {
    findRegistrations: function (hospitalId, conditions, page) {
        ...
        sql = !conditions.length ? sql : 'select SQL_CALC_FOUND_ROWS ot.name as outPatientServiceTypeName,r.content,r.transferDoctor,r.transferDoctorName,r.outPatientServiceType,r.id,mo.orderNo,p.id as patientId, p.patientBasicInfoId,r.outpatientStatus,p.counselor, r.registerDate, s.`name` as shiftPeriod, p.medicalRecordNo,r.patientMobile,r.patientName,r.gender, p.balance,p.source, p.memberCardNo,r.memberType, r.doctorName, r.`comment`, r.registrationFee, r.registrationType,r.departmentName, po.address,r.`status`, e.`name` as creatorName,r.createDate, r.outPatientType, r.status, r.sequence, r.businessPeopleName from Registration r left JOIN ShiftPeriod s ON s.id= r.shiftPeriod left join Patient p on p.id=r.patientId left join Doctor d on d.id=r.doctorId left join MedicalOrder mo on mo.registrationId= r.id left join PatientBasicInfo po on po.id = p.patientBasicInfoId left JOIN Employee e on e.id = r.creator left join OutpatientServiceType ot on ot.id=r.outPatientServiceType LEFT JOIN Doctor dd on dd.id = r.doctorId  where r.status <>4 and r.hospitalId = ? and ' + conditions.join(' and ') + ' order by r.id desc limit ?, ?';
