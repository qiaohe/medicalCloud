module.exports = {
    ...
    notification: {
        ...
        findPatientQueueBy: 'select doctorId, doctorName, r.departmentName, d.clinic, patientName, sequence, dep.floor from Registration r LEFT JOIN Doctor d on d.id = r.doctorId left JOIN Department dep on dep.id = r.departmentId where r.id=?',
    ...
    order: {
        ...
        findByOrderNo: 'select m.comment, m.discountRate, m.cardCharge, m.registrationId, m.type,m.orderNo,m.createDate, m.amount, m.paymentAmount,r.nurseName, r.patientName,r.patientMobile,  r.departmentName, r.doctorName,r.patientId,m.invoiceSequenceNo, r.hospitalName, m.paidAmount1, m.paidAmount2, m.paidAmount3, m.paymentType1,m.paymentType2, m.paymentType3, m.paidAmount, m.chargedByName, m.chargeDate, m.invoiceSequenceNo, r.businessPeopleId from MedicalOrder m left join Registration r on m.registrationId = r.id where m.hospitalId= ? and m.orderNo=?',
