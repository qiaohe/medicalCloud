module.exports = {
    ...
    registration: {
        ...
        findAppointments: 'select  SQL_CALC_FOUND_ROWS a.*,sp.`name` as periodName, p.medicalRecordNo, p.memberCardNo, p.memberType, p.source, po.`name`, po.realName, po.gender,po.mobile from Appointment a left JOIN Patient p on p.id = a.patientId left join PatientBasicInfo po on po.id = p.patientBasicInfoId left join ShiftPeriod sp on sp.id = a.period where a.hospitalId=? ',
