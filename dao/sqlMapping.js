module.exports = {
    ...
    doctor: {
        ...
        findHistoryOutpatients: 'select SQL_CALC_FOUND_ROWS ot.name as outPatientServiceTypeName, concat(DATE_FORMAT(r.registerDate, \'%Y-%m-%d \') , sp.`name`) as registerDate, r.id,r.outPatientServiceType, r.patientName, r.patientMobile, r.gender, r.age, r.sequence, r.registrationType, r.`comment`, r.outPatientType, r.createDate, r.businessPeopleName as recommender, r.outpatientStatus from Registration r LEFT JOIN  PatientBasicInfo p on p.id = r.patientBasicInfoId left JOIN ShiftPeriod sp on sp.id = r.shiftPeriod LEFT JOIN Doctor d ON d.id = r.doctorId left join OutpatientServiceType ot on ot.id=r.outPatientServiceType where d.employeeId = ? order by r.registerDate desc , r.sequence',
