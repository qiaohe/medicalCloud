module.exports = {
    ...
    order: {
        ...
        findOrdersByStatus: 'select SQL_CALC_FOUND_ROWS m.*, p.medicalRecordNo, mo.paidAmount as totalPaidAmount, r.nurseName,r.patientName, r.departmentId, r.departmentName,r.patientMobile,r.memberType, r.hospitalId, r.hospitalName, r.doctorId, r.doctorName,r.patientId from MedicalOrder m left join Registration r on m.registrationId = r.id left join MedicalOrder mo on mo.orderNo=m.referenceOrderNo left join Patient p on p.id = r.patientId where m.hospitalId= ? ',
        findOrdersBy: 'select SQL_CALC_FOUND_ROWS  m.*, r.nurseName,r.patientName,r.patientMobile,r.memberType, r.departmentId, r.departmentName, r.hospitalId, r.hospitalName, r.doctorId, r.doctorName,r.patientId from MedicalOrder m left join Registration r on m.registrationId = r.id where m.hospitalId= ? ',
        ...
        findOrdersByType: 'select SQL_CALC_FOUND_ROWS m.*, r.patientName,r.nurseName,r.patientMobile,r.memberType,r.departmentId,r.patientId, r.departmentName, r.hospitalId, r.hospitalName, r.doctorId, r.doctorName from MedicalOrder m left join Registration r on m.registrationId = r.id where m.hospitalId=? and m.type=? ',
        ...
        findByOrderNos: 'select m.comment,m.discountRate,m.referenceOrderNo,m.unPaidAmount, m.registrationId, m.type,m.orderNo,m.createDate, m.amount, m.paymentAmount, r.patientName,r.patientMobile, p.balance,r.nurseName, r.departmentName, r.doctorName,r.patientId from MedicalOrder m left join Registration r on m.registrationId = r.id left JOIN Patient p ON p.id = r.patientId where m.hospitalId= ? and m.orderNo in ',
        findByOrderNo: 'select m.comment, m.discountRate, m.cardCharge, m.registrationId, m.type,m.orderNo,m.createDate, m.amount, m.paymentAmount,m.unPaidAmount,r.nurseName, r.patientName,r.patientMobile,  r.departmentName, r.doctorName,r.patientId,m.invoiceSequenceNo, r.hospitalName, m.paidAmount1, m.paidAmount2, m.paidAmount3, m.paymentType1,m.paymentType2, m.paymentType3, m.paidAmount, m.chargedByName, m.chargeDate, m.invoiceSequenceNo, r.businessPeopleId from MedicalOrder m left join Registration r on m.registrationId = r.id where m.hospitalId= ? and m.orderNo=?',
        findOrderByOrderNo: 'select r.patientBasicInfoId,r.nurseName, r.patientMobile, h.patientName, r.hospitalName, r.departmentName, r.doctorName,r.sequence from MedicalOrder m left join Registration r on m.registrationId = r.id left JOIN MedicalHistory h on h.registrationId = m.registrationId where m.orderNo=?',
