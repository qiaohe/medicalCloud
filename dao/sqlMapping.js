module.exports = {
    ...
    doctor: {
        ...
        findHistoryOutpatients: 'select SQL_CALC_FOUND_ROWS ot.name as outPatientServiceTypeName,r.patientId, r.registerDate, r.content, r.doctorName,r.transferDoctorName, r.id,r.outPatientServiceType, r.patientName, r.patientMobile, r.gender, r.age, r.sequence, r.registrationType, r.`comment`, r.outPatientType, r.createDate, r.businessPeopleName as recommender, r.outpatientStatus from Registration r LEFT JOIN  PatientBasicInfo p on p.id = r.patientBasicInfoId  LEFT JOIN Doctor d ON d.id = r.doctorId left join OutpatientServiceType ot on ot.id=r.outPatientServiceType where d.employeeId = ? order by r.registerDate desc , r.sequence',
    ...
    registration: {
        statByDoctorAchievement: 'select r.doctorId, r.doctorName, count(*) as count, sum(paidAmount) as paidAmount, sum(paidAmount1) as paidAmount1 , sum(paidAmount2) as paidAmount2, sum(paidAmount3) as paidAmount3,m.paymentType, paymentType1, paymentType2, paymentType3 from MedicalOrder m left join Registration r on r.id = m.registrationId where m.hospitalId=? and r.doctorId is not NULL ',
        statByNurseAchievement: 'select m.nurse, m.nurseName, r.doctorId, r.doctorName, sum(paidAmount) as paidAmount from MedicalOrder m left join Registration r on r.id = m.registrationId where m.hospitalId=? and  m.nurse is not NULL and paidAmount > 0 ',
        statByChargeItemSummary: 'select r.doctorId, r.doctorName,d.id, d.`name`, sum(p.totalPrice) as amount from Prescription p left join Registration r on r.id = p.registrationId left JOIN ChargeItem c on c.id = p.chargeItemId LEFT JOIN DrugCategory d on d.id = c.category where p.hospitalId=? and d.id is not NULL and r.doctorId is not NULL ',
        sumPrepaidHistories: 'select sum(paidAmount) as amount from PrepaidHistory',
        statByChargeItem: 'select d.id, d.`name`, r.doctorId, r.doctorName, sum(p.quantity) as quantity, sum(p.totalPrice) as amount from Prescription p left join Registration r on r.id = p.registrationId left JOIN ChargeItem c on c.id = p.chargeItemId LEFT JOIN DrugCategory d on d.id = c.category where p.hospitalId=? and d.id is not NULL ',
        statByChargeItemByNurse: 'select d.id, d.`name`, m.nurse, m.nurseName, sum(p.quantity) as quantity, sum(p.totalPrice) as amount from Prescription p left join MedicalOrder m on m.orderNo = p.orderNo left JOIN ChargeItem c on c.id = p.chargeItemId LEFT JOIN DrugCategory d on d.id = c.category where p.hospitalId=? and d.id is not NULL and m.nurse is not NULL ',
        findOrdersWithChargeBy: 'select chargedBy, chargedByName, paidAmount, paidAmount1, paidAmount2, paidAmount3, paymentType, paymentType1, paymentType2, paymentType3 from MedicalOrder where hospitalId=? and chargedBy is not NULL and `status` in (1,3) and (paidAmount>0 or paidAmount1>0 or paidAmount2>0 or paidAmount3>0) ',
    ...
    patient: {
        ...
        findPatients: 'select SQL_CALC_FOUND_ROWS p.medicalRecordNo, pb.address,pb.idCard, p.counselor,pb.birthday,p.id, pb.`realName`,pb.createDate, pb.gender, pb.headPic,pb.birthday, pb.mobile, p.memberCardNo,p.memberType, dic.value as source,e.`name` as recommenderName,p.consumptionLevel, gc.`name` as groupName, pb.age,p.groupId from Patient p left JOIN SalesMan e on e.id = p.recommender left JOIN GroupCompany gc on gc.id = p.groupId left join PatientBasicInfo pb on pb.id = p.patientBasicInfoId left join Dictionary dic on dic.id = p.source and dic.type=9 where p.hospitalId =? order BY p.createDate desc limit ?, ?',
    ...
    notification: {
        ...
        findPatientQueue: 'select doctorId, doctorName, r.departmentName, d.clinic, patientName, sequence from Registration r LEFT JOIN Doctor d on d.id = r.doctorId left JOIN Department dep on dep.id = r.departmentId left JOIN Hospital h ON h.id = r.hospitalId where r.registerDate = ? and d.floor = ? and h.domainName= ? and r.sequence is not null and (r.outpatientStatus =0 or r.outpatientStatus = 5) and r.status <>4 and r.doctorId is not null order by doctorId, sequence',
        findPatientQueueByDepartmentId: 'select doctorId, doctorName, r.departmentName, d.clinic, patientName, sequence, d.floor from Registration r LEFT JOIN Doctor d on d.id = r.doctorId left JOIN Department dep on dep.id = r.departmentId where r.registerDate = ? and dep.id = ? and r.sequence is not null and (r.outpatientStatus =0 or r.outpatientStatus = 5) and r.status <>4 order by doctorId, sequence',
        findPatientQueueBy: 'select doctorId, doctorName, r.departmentName, d.clinic, patientName, sequence, d.floor from Registration r LEFT JOIN Doctor d on d.id = r.doctorId left JOIN Department dep on dep.id = r.departmentId where r.id=?',
    ...
    medical: {
        ...
        findPrescriptionsBy: 'select r.*, m.`status`, m.paidAmount, m.payableAmount, m.unPaidAmount from Prescription r left JOIN MedicalOrder m on m.orderNo = r.orderNo where r.registrationId = ?',
        ...
        findPrescriptionsByOrderNoPageable: 'select SQL_CALC_FOUND_ROWS p.*, m.status from Prescription p left JOIN MedicalOrder m on m.orderNo = p.orderNo where p.orderNo = ? and p.isAppend =1 order by p.createDate desc limit ?,?',
        ...
        summaryChargeItems: 'select SQL_CALC_FOUND_ROWS p.id, p.name, p.price, sum(p.quantity) as totalQuantity, p.orderNo, p.createDate, sum(receivable) as totalAmount, r.doctorName, r.nurseName,o.referenceOrderNo, r.patientId, r.patientName from Prescription p left join Registration r on r.id = p.registrationId  left join ChargeItem c on c.id = p.chargeItemId left join MedicalOrder o on o.orderNo = p.orderNo where p.hospitalId = ? ',
        sumChargeItems: 'select sum(p.quantity) as totalQuantity, sum(receivable) as totalAmount from Prescription p left join ChargeItem c on c.id = p.chargeItemId left join Registration r on r.id = p.registrationId join MedicalOrder o on o.orderNo = p.orderNo where p.hospitalId = ? ',
    ...
    order: {
        ...
        updateTotalPrice: 'update MedicalOrder set amount = amount + ?, paymentAmount=amount, payableAmount=amount  where orderNo = ?',
        ...
        findOrdersByStatus: 'select SQL_CALC_FOUND_ROWS m.*, p.medicalRecordNo, mo.paidAmount as totalPaidAmount, r.patientName, r.departmentId, r.departmentName,r.patientMobile,r.memberType, r.hospitalId, r.hospitalName, r.doctorId, r.doctorName,r.patientId from MedicalOrder m left join Registration r on m.registrationId = r.id left join MedicalOrder mo on mo.orderNo=m.referenceOrderNo left join Patient p on p.id = r.patientId where m.hospitalId= ? ',
        ...
        findByOrderNos: 'select m.comment,m.discountRate,m.referenceOrderNo,m.unPaidAmount, m.registrationId, m.type,m.orderNo,m.createDate, m.amount, m.paymentAmount, r.patientName,r.patientMobile, p.balance,m.nurseName, r.departmentName, r.doctorName,r.patientId from MedicalOrder m left join Registration r on m.registrationId = r.id left JOIN Patient p ON p.id = r.patientId where m.hospitalId= ? and m.orderNo in ',
        findByOrderNo: 'select m.comment, m.discountRate, m.cardCharge, m.registrationId, m.type,m.orderNo,m.createDate, m.amount, m.paymentAmount,m.unPaidAmount,m.nurseName, r.patientName,r.patientMobile,  r.departmentName, r.doctorName,r.patientId,m.invoiceSequenceNo, r.hospitalName, m.paidAmount1, m.paidAmount2, m.paidAmount3, m.paymentType1,m.paymentType2, m.paymentType3, m.paidAmount, m.chargedByName, m.chargeDate, m.invoiceSequenceNo, r.businessPeopleId from MedicalOrder m left join Registration r on m.registrationId = r.id where m.hospitalId= ? and m.orderNo=?',
        findOrderByOrderNo: 'select r.patientBasicInfoId,m.nurseName, r.patientMobile, h.patientName, r.hospitalName, r.departmentName, r.doctorName,r.sequence from MedicalOrder m left join Registration r on m.registrationId = r.id left JOIN MedicalHistory h on h.registrationId = m.registrationId where m.orderNo=?',
